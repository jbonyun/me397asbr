function joint_angles = redundancy_resolution_2(robot, guess_joint_angles, Tsd)
    ...
    cond_num = J_condition(J_body(robot, joint_angles));
        %w0=sqrt(det(J_body(robot, joint_angles)*J_body(robot, joint_angles)'));
    ...
    while norm(twist_b(1:3)) > angular_thresh || norm(twist_b(4:6)) > linear_thresh
        ...
        step = J_daggr*twist_b*0.1+(eye(7,7)-J_daggr*Jb)*q0_dot';
